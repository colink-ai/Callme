...
Callme 侧只保留进程级共享的 Helios Engine/Registry、领域隔离、会话落库、WebSocket 转发和业务展示。Hermes 的启动参数、环境变量、`config.yaml` 结构、MCP 渲染等基础 Agent 细节由 Helios adapter 负责；Callme 如需注入业务开关，只通过 Helios 提供的配置扩展点完成。

当前 UI 已消费文本、思考、工具调用、工具结果、usage 和 done 等基础流式块；Artifact、Handoff、Question、Permission、计划进度和富工具结果已经在应用 DTO 中保留承载字段，但业务页面尚未完整呈现。进入告警处置、T2T 转交、产物归档或人在环审批阶段时，应优先把这些 Helios 语义事件接入 Callme 的工单、审计和会话 UI，而不是重新定义一套运行时协议。
